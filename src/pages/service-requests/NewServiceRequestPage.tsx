...
export function NewServiceRequestPage({ onNotify }: NewServiceRequestPageProps) {
  ...
  const createRequest = (status: ServiceRequestStatus) => {
    ...
    const request: ServiceRequest = {
      ...
      priority: form.priority as ServicePriority, assignedMarketingRepresentative: form.assignedRepresentative, requestedOperationsReviewer: form.operationsReviewer || undefined, internalTags: form.internalTags.split(',').map((tag) => tag.trim()).filter(Boolean), internalNotes: form.internalNotes || undefined, followUpDate: form.followUpDate || undefined, status, createdAt: now, updatedAt: now,
    }
  ...
  return <div className="new-request-page">
    ...
    <form onSubmit={(event: FormEvent) => { event.preventDefault(); createRequest('Under Review') }} noValidate>
      ...
      <RequestSection icon={Ship} title="Vessel Information" description="Record vessel identity, dimensions, and operating particulars."><div className="request-form-grid">
      ...
      <RequestSection icon={ShieldCheck} title="Operational Requirements" description="Capture assistance, safety, hazards, and operating instructions."><div className="request-form-grid">

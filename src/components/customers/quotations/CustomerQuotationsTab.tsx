...
export function CustomerQuotationsTab({ customer, onNotify }: Props) {
  ...
  return <section className="quotations-tab" aria-labelledby="customer-quotations-title">
    ...
    <div className="quotation-summary" aria-label="Quotation summary"><div><span>Total Quotations</span><strong>{quotations.length}</strong></div><div><span>Awaiting Response</span><strong>{rows.filter((row) => ['Sent', 'Viewed'].includes(row.effectiveStatus)).length}</strong></div><div><span>Customer Approved</span><strong>{rows.filter((row) => row.effectiveStatus === 'Customer Approved').length}</strong></div><div><span>Expiring Soon</span><strong>{expiringSoon}</strong></div></div>

...
import type { Customer, CustomerActivity, CustomerInternalNote } from '../../types/customer'
...
export function CustomerOverviewTab({ customer, appointment, activities, notes, onNotify }: CustomerOverviewTabProps) {
  ...
  return <div className="overview-grid">
    ...
    <section className="profile-card overview-appointment"><header><CardHeading icon={CalendarClock} title="Upcoming Appointment" eyebrow="Next engagement" /></header>{appointment ? <div className="appointment-content"><div className="appointment-hero"><div className="appointment-date"><strong>{new Intl.DateTimeFormat('en-PH', { day: '2-digit' }).format(new Date(appointment.startAt))}</strong><span>{new Intl.DateTimeFormat('en-PH', { month: 'short' }).format(new Date(appointment.startAt))}</span></div><div><span className="appointment-status">{appointment.status}</span><h3>{appointment.type}</h3><p><CalendarClock size={13} />{new Intl.DateTimeFormat('en-PH', { hour: 'numeric', minute: '2-digit' }).format(new Date(appointment.startAt))}</p><p><MapPin size={13} />{appointment.location ?? appointment.meetingMethod}</p></div></div><DetailList details={[[ 'Contact ID', appointment.contactId ], [ 'Assigned Representative', appointment.assignedRepresentativeName ], [ 'Date', new Intl.DateTimeFormat('en-PH', { month: 'long', day: 'numeric', year: 'numeric' }).format(new Date(appointment.startAt)) ], [ 'Status', appointment.status ]]} /></div> : <div className="compact-empty"><CalendarDays size={24} /><p>No upcoming appointments.</p><button className="button button-secondary" type="button" onClick={() => onNotify('Open the Appointments tab to schedule an appointment.')}>Schedule Appointment</button></div>}</section>

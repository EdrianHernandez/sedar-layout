...
export function ContactStatusBadge({ status }: { status: ContactStatus }) { return <span className={`contact-status-badge contact-${status.toLowerCase()}`} aria-label={`Status: ${status}`}>{status}</span> }
